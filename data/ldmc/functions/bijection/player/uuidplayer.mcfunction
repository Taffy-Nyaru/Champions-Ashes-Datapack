...
execute on origin if entity @s[nbt=!{SelectedItem:{tag:{id:"ldmc:sonic_boom_shard"}}}] run tp @e[tag=a] 0 -100 0
#将匹配到uuid的移出队列
data remove storage math:io playerlist[]
scoreboard players remove #uuid uuidplayer 1

#递归循环检测
execute if score #uuid uuidplayer matches 1.. run function ldmc:bijection/player/uuidplayer
