...
execute unless entity @s[nbt={SelectedItem:{tag:{LodestoneTracked:1b}}}] if score @s[nbt={SelectedItem:{tag:{id:"ldmc:homeward_bone"}}}] usedBone matches 1.. as @e[tag=crying_waystone,tag=raycast.target] run tag @s add activated
