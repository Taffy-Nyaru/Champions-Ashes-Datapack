...
execute if entity @a[sort=nearest,distance=..10,gamemode=!creative] at @s run loot spawn ~ ~ ~ loot ldmc:blocks/crafting_table
#kill @e[type=item,distance=..3,tag=ldmc_block_drop]
setblock ~ ~ ~ air destroy
kill @e[type=item,sort=nearest,nbt={Item:{id:"minecraft:dropper",}},nbt=!{Item:{tag:{id:"ldmc:crafting_table"}}},limit=1,distance=..5]
kill @s