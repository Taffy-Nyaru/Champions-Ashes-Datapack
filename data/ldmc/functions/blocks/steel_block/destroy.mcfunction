...
execute if entity @a[sort=nearest,distance=..10,gamemode=!creative] at @s run loot spawn ~ ~ ~ loot ldmc:blocks/steel_block
#kill @e[type=item,distance=..3,tag=ldmc_block_drop]
setblock ~ ~ ~ air replace
kill @e[type=item,sort=nearest,nbt={Item:{id:"minecraft:iron_block",}},nbt=!{Item:{tag:{id:"ldmc:steel_block"}}},limit=1,distance=..5]
kill @s