execute unless entity @a[tag=parry_success,distance=..10] run scoreboard players add #elder_parrying_time ldmc_Timer 1

execute if score #elder_parrying_time ldmc_Timer matches 1 run data modify entity @s Invulnerable set value 1b
execute if score #elder_parrying_time ldmc_Timer matches 1 unless entity @e[tag=parry_shield,distance=..5] run summon item_display ^ ^1.5 ^1 {Tags:["parry_shield"],item:{id:"shield",Count:1b,tag:{CustomModelData:1390050,Enchantments:[{id:"infinity",lvl:1}]}}}
...
execute as @e[type=item_display,tag=parry_shield] run data modify entity @s transformation.left_rotation set from storage math:io rotate_shield
...
execute if score #elder_parrying_time ldmc_Timer matches 0 run data modify storage generic:main nearest_attacker set value [I;-929217864,-783792174,-1513004625,-1275487381]
