...
execute if score @s skill_time matches 1 run item modify entity @s[gamemode=survival] weapon.mainhand ldmc:pkcs_damage
...
execute if score @s skill_time matches 1..24 run scoreboard players add @e[type=!#ldmc:special_entities,distance=..5,tag=!using_skill] frostbite_Timer 120
execute if score @s skill_time matches 1..24 as @e[type=!#ldmc:special_entities,distance=..5,tag=!ally,tag=!using_skill] run damage @s 1 ldmc:frostbite by @a[limit=1,sort=nearest,tag=using_skill]
