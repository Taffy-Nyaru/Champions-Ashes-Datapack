...
execute store result score @s thunderarrow_damage run scoreboard players operation @s power_level += #2 ldmcValue

execute if data storage ldmc:player_data {SelectedItem:{tag:{id:"ldmc:thunder_slayer"}}} if score @s usedBow matches 1.. run function ldmc:items/thunder_slayer/used_bow
...
execute if entity @e[tag=aj.thunder_bullet.root] run scoreboard players add #thunder_timer ldmc_Timer 1
execute if score #thunder_timer ldmc_Timer matches 60.. as @e[tag=aj.thunder_bullet.root] at @s at @e[type=!#ldmc:special_entities,type=!villager,type=!player,type=!iron_golem,distance=..12] run summon lightning_bolt ~ ~ ~ {Tags:["Thunder"]}
execute if score #thunder_timer ldmc_Timer matches 60.. run scoreboard players set #thunder_timer ldmc_Timer 0
...
execute as @e[tag=aj.thunder_bullet.root] at @s if entity @e[distance=..5,type=!#ldmc:special_entities,predicate=ldmc:hurttime,predicate=!ldmc:using_item/thunder_slayer] run function ldmc:items/thunder_slayer/effect