...
execute as @e[type=interaction,tag=fumo_clicker,scores={block_ca.interaction_rClick=1..}] run data modify entity @s interaction.timestamp set value 0
execute as @e[type=interaction,tag=fumo_clicker,scores={block_ca.interaction_lClick=1..}] run data modify entity @s attack.timestamp set value 0