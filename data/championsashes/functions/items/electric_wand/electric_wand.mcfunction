...
execute unless entity @s[tag=electric_beacon_skill_cooling] if entity @e[type=interaction,distance=..10,tag=clicker,scores={ca.interaction_rClick=1..}] as 2094fd65-93de-4a69-9ea2-a74c7f35bc75 on origin if entity @s[tag=!stiff] at @e[distance=..50,limit=1,sort=nearest,type=!#championsashes:special_entities,type=!player] run function championsashes:items/electric_wand/skill

function championsashes:items/electric_wand/electric_beacon
execute if entity @s[tag=electric_beacon_skill_cooling] run function championsashes:items/electric_wand/cooling

execute at @s as @e[type=item_display,tag=aj.electric_beacon.root,limit=1,sort=nearest] unless entity @e[type=!#championsashes:special_entities,distance=..1] run ride @s mount @e[type=!#championsashes:special_entities,type=!player,limit=1,sort=nearest]
execute at @s as @e[type=item_display,tag=aj.electric_beacon.root,limit=1,sort=nearest] unless entity @e[type=!#championsashes:special_entities,distance=..1] run ride @s mount @e[type=!#championsashes:special_entities,type=!player,limit=1,sort=nearest]
