...
execute if entity @e[type=interaction,distance=..10,tag=clicker,scores={ca.interaction_rClick=1..}] as 2094fd65-93de-4a69-9ea2-a74c7f35bc75 on origin if entity @s[tag=!stiff] if score @s shift matches 1.. at @s unless entity @e[tag=aj.wave_of_destruction.root,distance=..2] run function championsashes:items/fume_ultra_greatsword/skill
...
execute if entity @e[type=interaction,distance=..10,tag=clicker,scores={ca.interaction_lClick=1..}] as a346a606-7102-4c7f-b48d-14038f3a5e56 on origin if entity @s[tag=!stiff] at @s unless entity @e[tag=aj.purple_fire.root,distance=..20] run function championsashes:items/fume_ultra_greatsword/purple_fire
