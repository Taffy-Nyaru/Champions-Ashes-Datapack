...
execute at @e[tag=aj.steel_anchor.locator,limit=1,tag=!player_camera] run summon creeper ~ ~ ~ {Invulnerable:1b,Fuse:0,powered:1b,Tags:["bomber"],ExplosionRadius:4}
...
execute as @a[limit=1,sort=nearest,tag=anchor_skill3] at @s unless block ~ ~5.5 ~ air unless block ~ ~5.5 ~ water run function championsashes:items/steel_anchor/back_to_inplace
