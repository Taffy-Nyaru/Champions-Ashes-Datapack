execute if entity @e[distance=..5,type=!#championsashes:special_entities,tag=parry_success] run effect give @s strength 5 4

execute at @s[scores={shift=1..}] unless entity @e[tag=raycast.target,type=glow_item_frame,distance=..2] run tag @s add needclicker
execute at @s[scores={shift=1..}] unless entity @e[tag=raycast.target,type=item_frame,distance=..2] run tag @s add needclicker
...
execute if entity @e[type=interaction,distance=..10,tag=clicker,scores={ca.interaction_rClick=1..}] as 2094fd65-93de-4a69-9ea2-a74c7f35bc75 on origin if entity @s[tag=!stiff] if score @s shift matches 1.. at @s unless entity @e[type=item_display,distance=..2,tag=aj.flame_of_lorian.root] run function championsashes:items/black_knight_greataxe/skill
