...
execute if entity @e[type=interaction,distance=..10,tag=clicker,scores={ca.interaction_rClick=1..}] as 2094fd65-93de-4a69-9ea2-a74c7f35bc75 on origin if entity @s[tag=!stiff] if score @s shift matches 1.. unless entity @e[distance=..2,type=item_display,tag=aj.rkss_cancel.root] run function championsashes:items/ringed_knight_straight_sword/skill
...
execute if entity @e[type=interaction,distance=..10,tag=clicker,scores={ca.interaction_lClick=1..}] as a346a606-7102-4c7f-b48d-14038f3a5e56 on origin at @s[tag=!stiff] unless entity @e[distance=..2,type=item_display,tag=aj.rkss_cancel.root] run function championsashes:items/ringed_knight_straight_sword/attack
