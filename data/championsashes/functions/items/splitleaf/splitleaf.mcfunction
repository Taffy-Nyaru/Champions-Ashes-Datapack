...
execute if entity @s[advancements={championsashes:hurt_entities=true}] at @s as @e[distance=..8,limit=1,type=!#championsashes:special_entities,tag=!ally,tag=raycast.target] run damage @s 15 minecraft:player_attack by @a[limit=1,sort=nearest,predicate=championsashes:using_item/splitleaf]
