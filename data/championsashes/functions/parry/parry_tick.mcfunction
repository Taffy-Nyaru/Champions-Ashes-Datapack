...
execute if score @s[advancements={championsashes:hurt_players_blocked=true}] parry_time matches 1..25 as @e[limit=1,sort=nearest,tag=raycast.target] on target if entity @s[type=player,tag=parry] run tag @e[tag=parry_target] add parry_success
execute if score @s parry_time matches 25.. run damage @s 1 minecraft:mob_attack by @e[tag=shield_breaker,limit=1,sort=nearest]
