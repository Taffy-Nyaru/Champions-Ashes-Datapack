...
execute at @e[distance=..80,type=marker,tag=aj.ancient_lightning_strike.locator] as @e[distance=..2,type=!#championsashes:special_entities,tag=!drakeblood_knight,type=!player] run function championsashes:animated_effects/ancient_lightning_bolt
execute at @e[distance=..80,type=marker,tag=aj.ancient_lightning_strike.locator] as @a[distance=..2,tag=!drakeblood_knight,gamemode=!creative,gamemode=!spectator] unless score @s ca.item.id_selecter_mainhand matches 999 run function championsashes:animated_effects/ancient_lightning_bolt
