...
execute as @s[type=!player,predicate=championsashes:entity_using_item/drakeblood_greatsword] on target if entity @s[type=player,advancements={championsashes:hurt_players=true}] run scoreboard players add @s bleeding_Timer 512
...
execute as @s[type=!player,predicate=championsashes:entity_using_item/moonlight_greatsword] on target run scoreboard players set @s custom_ca.entity.stiff_time 5
