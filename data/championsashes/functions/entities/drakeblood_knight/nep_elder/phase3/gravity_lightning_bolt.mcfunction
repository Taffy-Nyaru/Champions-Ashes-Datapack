...
execute at @e[tag=aj.nep_fume_ugs_skills.locator,type=marker] as @e[type=!#championsashes:special_entities,tag=nep_foe,tag=!drakeblood_knight,distance=..3] run damage @s 5 minecraft:outside_border by @e[tag=nep_elder,limit=1,sort=nearest,type=wither_skeleton]