...
execute unless score @s[tag=!nep_switched_to_ugs] nep_phase3_tp_buffer matches 1.. if entity @p[distance=..8,gamemode=!creative,gamemode=!spectator] run function championsashes:entities/drakeblood_knight/nep_elder/phase3/phase3_teleport
...
#A fucking teleport command costs 1tick+ duration to fully execute
...
execute unless entity @e[tag=nep_space_based_laser,type=item_display,limit=1,sort=nearest] unless entity @e[tag=aj.nep_fume_ugs_skills.root,type=item_display,limit=1,sort=nearest] if score @s nep_phase3_switch_to_dragonslayer matches 200.. run function championsashes:entities/drakeblood_knight/nep_elder/phase3/switch_to_dragonslayer
...
execute if entity @s[tag=!nep_switched_to_ugs] unless score @s nep_phase3_tp_buffer matches 1..6 facing entity @e[type=item_display,tag=nep_flight_marker_chasing,limit=1,sort=nearest,distance=..50] feet run tp @s ^ ^ ^0.2
