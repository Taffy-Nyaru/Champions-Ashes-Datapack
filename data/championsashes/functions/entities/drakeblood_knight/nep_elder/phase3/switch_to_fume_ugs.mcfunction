...
execute if score change_weapon_inventory math_output matches -1024..1024 as @e[tag=aj.nep_fume_ugs_skills.root,type=item_display,distance=..50] run function animated_java:nep_fume_ugs_skills/animations/nep_supernova_throw/play
execute unless score change_weapon_inventory math_output matches -1024..1024 run ride @s mount @e[tag=nep_ride,type=item_display,limit=1,sort=nearest]
