execute unless entity @e[type=!#championsashes:special_entities,tag=nep_foe,type=!player,distance=..10] facing entity @e[tag=nep_elder,type=wither_skeleton,limit=1,sort=nearest] feet run tp @s ^ ^ ^-0.4 ~ ~
...
execute if entity @e[distance=..1.5,type=!#championsashes:special_entities,tag=nep_foe,limit=1,sort=nearest] run kill @s

#When enter block
execute unless block ~ ~0.8 ~ #championsashes:transparent_blocks run function championsashes:entities/drakeblood_knight/nep_elder/weapon_abilities/moonlight_slash_hostile_damage
execute unless block ~ ~0.8 ~ #championsashes:transparent_blocks run kill @s
