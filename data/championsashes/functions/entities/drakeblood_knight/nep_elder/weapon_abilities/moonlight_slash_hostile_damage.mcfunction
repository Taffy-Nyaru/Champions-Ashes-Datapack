...
tag @s add stiff
execute at @s if entity @e[distance=..2.5,type=!#championsashes:special_entities,tag=nep_foe,limit=1,sort=nearest] unless entity @e[tag=aj.ice_crystal.root,type=item_display,distance=..2] facing ~ ~ ~ run function animated_java:ice_crystal/summon
execute at @s if entity @e[distance=..2.5,type=!#championsashes:special_entities,tag=nep_foe,limit=1,sort=nearest] as @e[tag=!nep_ice_crystal_played_anim,tag=aj.ice_crystal.root,type=item_display,distance=..2,limit=1,sort=nearest] run function animated_java:ice_crystal/animations/ice_crystallization/play

#Ice Storm
execute at @s if score @s frostbite_Timer matches 4200.. run function animated_java:zamor_ice_storm/summon
