...
execute as @e[tag=lazer_target,type=!villager,type=!iron_golem] run damage @s 1 championsashes:cheat_engine by @a[tag=lazer_owner,limit=1,sort=nearest]
execute as @e[tag=start_entity,tag=!lazer_owner,type=!villager,type=!iron_golem] run damage @s 1 championsashes:cheat_engine by @a[tag=lazer_owner,limit=1,sort=nearest]

data modify entity @s transformation.scale set from storage generic:main lazer_beam_length
execute at @s if score @s lazer_fading matches ..0 run function championsashes:projectiles/lazer_beam/lazer_resummon